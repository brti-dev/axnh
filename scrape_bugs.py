...
january_column_num = 5              # The column number where the months begin; 0=Leftmost column

# File locations
images_dir = 'static/images/bugs'
source_url = 'https://animalcrossing.fandom.com/wiki/Bugs_(New_Horizons)'
file_html = 'data/bugs_{}.html'
file_data = 'data/bugs_{}.json'

if get_from_source is True:
    source = requests.get(source_url).text
    s = BeautifulSoup(source, 'html.parser')
    table = {
        'north': s.find_all('table', attrs={'class':'sortable'})[0],
        'south': s.find_all('table', attrs={'class':'sortable'})[1]
    }
    for hemisphere in hemispheres:
        print('Writing to', file_html.format(hemisphere))
